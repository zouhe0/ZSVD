    ...
    if not args.skip_train:
        if args.process_model == 0:
            ...
            with ThreadPoolExecutor() as executor:
                future_SDE = executor.submit(subprocess.run, SDE_train_cmd, check=True)
                future_pre_train = executor.submit(subprocess.run, pre_train_cmd, check=True)
            try:
                future_SDE.result()
                future_pre_train.result()
...
def main():
    ...
    parser.add_argument("--device", type=str, default="cuda:2", help="训练设备 (cuda/cpu)")
    ...
    parser.add_argument('--SDE_epochs', type=int, default=45, help='SDE网络训练轮数')
    ...
    parser.add_argument("--pre_lr",type=float, default=0.015, help="预训练学习率")
