...
def train(training_data_loader, name):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(training_data_loader):
            ...
            optimizer.zero_grad()
            lms_rr = wald_protocol_v1(lms,pan, 4, sensor)
            pan_rr = wald_protocol_v2(ms, pan, 4, sensor)
            # print(pan_rr.shape)
            #lms_rr = torch.rand(1, 8, 128, 128)
            #pan_rr = torch.rand(1,1,128,128)
