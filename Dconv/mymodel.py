...
class FusionNet_new(nn.Module):
    def __init__(self):
        ...
        # 第二个残差模块
        self.depthwise2 = nn.Conv2d(hidden_channels, hidden_channels, kernel_size=3, 
                                   padding=1, groups=hidden_channels, bias=False)
        self.pointwise2 = nn.Conv2d(hidden_channels, hidden_channels, kernel_size=1, bias=True)
        ...
        # 初始化权重
        init_weights(self.conv1, self.depthwise, self.pointwise, self.depthwise2, self.pointwise2, self.conv3)
    ...
    def forward(self, x, y):
        ...
        res = rs
        rs = self.relu(self.depthwise2(rs))
        rs = self.relu(self.pointwise2(rs))
        rs = rs + res  # 第二个残差连接
